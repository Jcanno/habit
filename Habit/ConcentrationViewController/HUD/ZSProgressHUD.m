...
{
    ZSProgressHUD *hud = [[self alloc] initWithFrame:[UIScreen mainScreen].bounds showText:showText HUDType:ZSHUDLoadingType];
...
+ (NSUInteger)hideAllHUDAnimated:(BOOL)animated
{
    ...
    {
        if (hud.type== ZSHUDpromptTextType) {
...
- (instancetype)initWithFrame:(CGRect)frame showText:(NSString *)showText HUDType:(ZSHUDType)type{
    ...
    if (self)
    {
        ...
        if (type==ZSHUDLoadingType) {
            //加载
            [self loadingAnimated];
        } else if (type==ZSHUDSuccessfulAnimatedType) {
            //成功
            [self pictureCorrect];
        } else if (type==ZSHUDErrorAnimatedType) {
            //失败效果
            [self picturError];
        } else if (type==ZSHUDpromptTextType) {
            //提示文字
            [self DpromptText];
        }
...
- (void)DpromptText {
    ...
    CGSize size = [MPublicManager workOutSizeWithStr:_tipText andFont:14 value:[NSValue valueWithCGSize:CGSizeMake(self.frame.size.width-40, 999)]];
    if (size.width>100) {
